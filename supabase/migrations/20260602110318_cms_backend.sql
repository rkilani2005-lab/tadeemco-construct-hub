...
-- Bilingual (ar/en) content store + structured collections, admin auth via
-- Supabase roles, public read / admin write RLS, and a Storage bucket for
-- images. Seeded from the site's existing static content.
-- ============================================================================
...
-- ---------------------------------------------------------------------------
-- 1. Roles & admin auth
--    The recommended Supabase pattern: roles live in their own table (never on
--    the user/JWT, to avoid privilege-escalation), checked by a SECURITY
--    DEFINER function so RLS policies can call it without recursion.
-- ---------------------------------------------------------------------------
do $$ begin
...
-- convenience: is the current caller an admin or editor?
...
-- Users can read their own roles; only admins manage roles.
...
-- ---------------------------------------------------------------------------
-- 2. Generic content store (all free-form page text + images)
--    key e.g. 'home.hero.title' ; field_type drives the admin widget.
-- ---------------------------------------------------------------------------
create table if not exists public.site_content (
  ...
  field_type  text not null default 'text',  -- text | textarea | image | icon
...
-- ---------------------------------------------------------------------------
-- 3. Navigation menu (items + icons + visibility + order)
-- ---------------------------------------------------------------------------
create table if not exists public.menu_items (
  ...
  icon        text default '',               -- lucide-react icon name (optional)
...
-- ---------------------------------------------------------------------------
-- 4. Services (title / tag / description / icon / image)
-- ---------------------------------------------------------------------------
create table if not exists public.services (
  id           uuid primary key default gen_random_uuid(),
  slug         text not null unique,          -- shoring | dewatering | waterproofing | excavation | ...
  ...
  icon         text default '',               -- maps to ServiceIcons key
...
create table if not exists public.projects (
  ...
  services      text[] not null default '{}',  -- service slugs
  image_url     text default '',
  is_visible    boolean not null default true,
  sort_order    int not null default 0,
  updated_at    timestamptz not null default now()
...
-- ---------------------------------------------------------------------------
-- 7. Contractors strip ("who works with us")
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 8. Per-route SEO meta
-- ---------------------------------------------------------------------------
create table if not exists public.seo_meta (
  id             uuid primary key default gen_random_uuid(),
  route          text not null unique,        -- '/', '/about', ...
...
-- ---------------------------------------------------------------------------
-- 9. Global site settings (single row, id = true)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 10. updated_at triggers for every table
-- ---------------------------------------------------------------------------
do $$
declare t text;
begin
...
-- ---------------------------------------------------------------------------
-- 11. RLS: public read, admin/editor write
-- ---------------------------------------------------------------------------
do $$
declare t text;
begin
...
-- ---------------------------------------------------------------------------
-- 12. Storage bucket for CMS images (public read, admin write)
-- ---------------------------------------------------------------------------
...
-- ============================================================================
-- SEED DATA (mirrors the existing static content so the live site is
-- unchanged on first load; admins edit from here).
-- ============================================================================
...
-- 13h. page content (free-form editable strings) ----------------------------
