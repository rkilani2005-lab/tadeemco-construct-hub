...
interface Row {
  ...
  logo_url: string; is_visible: boolean; sort_order: number;
}
const clean = (d: Record<string, unknown>): Row => ({
  id: d.id as string,
  name_ar: (d.name_ar as string) ?? '', name_en: (d.name_en as string) ?? '',
  logo_url: (d.logo_url as string) ?? '', is_visible: (d.is_visible as boolean) ?? true,
  sort_order: (d.sort_order as number) ?? 0,
});

export const ContractorsEditor = () => {
  const [rows, setRows] = useState<Row[]>([]);
  const [removed, setRemoved] = useState<string[]>([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);

  const load = () => supabase.from('contractors').select('*').order('sort_order').then(({ data }) => {
  ...
  const addRow = () => setRows((rs) => [...rs, clean({
    id: crypto.randomUUID(), is_visible: true, sort_order: rs.length + 1,
  })]);
  ...
  const save = async () => {
    setSaving(true);
    if (removed.length) await supabase.from('contractors').delete().in('id', removed);
    const { error } = await supabase.from('contractors').upsert(rows.map((r, i) => ({ ...r, sort_order: i + 1 })));
    setSaving(false);
    if (error) { toast.error(error.message); return; }
    toast.success('Contractors saved'); setRemoved([]); load();
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-black text-foreground">Contractors</h1>
      ...
      <div className="space-y-4">
        {rows.map((r, i) => (
          <div key={r.id} className="space-y-4 rounded-lg border border-border bg-white p-5">
            <div className="flex items-center justify-between">
              <span className="text-sm font-semibold text-foreground">{r.name_en || 'New contractor'}</span>
            ...
            <ImageUploader label="Logo" value={r.logo_url} folder="contractors"
              onChange={(url) => set(r.id, 'logo_url', url)} />
